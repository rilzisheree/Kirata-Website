...
export interface BackgroundMusicHandle {
  /** Call this synchronously inside a user-gesture handler to unlock audio on iOS. */
  play: () => void;
}
...
export const BackgroundMusic = forwardRef<BackgroundMusicHandle, BackgroundMusicProps>(
  function BackgroundMusic(_props, ref) {
    const playerRef    = useRef<any>(null);
    const mountNodeRef = useRef<HTMLDivElement>(null);
    const [volume, setVolume] = useState(25);
    const [ready, setReady]   = useState(false);
    const readyRef    = useRef(false);
    const pendingPlay = useRef(false); // set true if play() called before onReady
    const [open, setOpen] = useState(false);
    const popupRef = useRef<HTMLDivElement>(null);

    // Expose play() so the parent can call it synchronously inside a click handler
    ...
    function fadeIn() {
      ...
      const interval = setInterval(() => {
        ...
        try { playerRef.current?.setVolume(Math.round(current)); } catch {}
        if (current >= target) clearInterval(interval);
      }, 75);
    }

    // Always init the YT player eagerly on mount (no autoplay) so it's ready by click time
    useEffect(() => {
      const initPlayer = () => {
